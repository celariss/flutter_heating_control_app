/// This file defines helpers to convert data from/to yaml format
/// (dart framework does not have a complete yaml implementation)
/// 
/// Authors: Jérôme Cuq
...
class YamlMapConverter {
  static dynamic _convertNode(dynamic v) {
    if (v is YamlMap) {
      return YamlMapConverter.toMap(v);
  ...
  static Map<String, dynamic> toMap(YamlMap yamlMap) {
    var map = <String, dynamic>{};
    for (var e in yamlMap.entries) {
      map[e.key.toString()] = YamlMapConverter._convertNode(e.value);
    }
    return map;
  }
}
